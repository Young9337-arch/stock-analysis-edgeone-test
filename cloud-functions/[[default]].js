...
// shared/server/server/market/symbol.js
var require_symbol = __commonJS({
  "shared/server/server/market/symbol.js"(exports, module) {
    "use strict";
...
// shared/server/server/market/parsers.js
var require_parsers = __commonJS({
  "shared/server/server/market/parsers.js"(exports, module) {
    "use strict";
    ...
    function parseTencentQuote(raw, symbol) {
      ...
      const book = [];
      for (let i = 0; i < 5; i++) book.push({ side: "bid", level: i + 1, price: numberAt(p, 9 + i * 2), volume: numberAt(p, 10 + i * 2) });
      for (let i = 0; i < 5; i++) book.push({ side: "ask", level: i + 1, price: numberAt(p, 19 + i * 2), volume: numberAt(p, 20 + i * 2) });
      return { symbol: symbol.code, market: symbol.market, name: p[1], price, previousClose, open: numberAt(p, 5), high, low, change: price - previousClose, changePercent: (price - previousClose) / previousClose * 100, volume: numberAt(p, 6), amount: (numberAt(p, 37) || 0) * 1e4, amplitude: high !== null && low !== null ? (high - low) / previousClose * 100 : null, turnoverRate: numberAt(p, 38), quoteTime: p[30] || "", book };
    }
    module.exports = { parseTencentQuote };
  }
});

// shared/server/server/market/quote.js
var require_quote = __commonJS({
  "shared/server/server/market/quote.js"(exports, module) {
    "use strict";
    var https = __require("https");
    var iconv = require_lib();
    var { parseTencentQuote } = require_parsers();
    function upstream(url) {
      return new Promise((resolve, reject) => {
        const req = https.get(url, { headers: { "User-Agent": "Mozilla/5.0 stock-analysis-local-proxy" }, timeout: 8e3 }, (res) => {
          let chunks = [];
          res.on("data", (c) => chunks.push(c));
          res.on("end", () => res.statusCode === 200 ? resolve(iconv.decode(Buffer.concat(chunks), "gbk")) : reject(new Error(`UPSTREAM_HTTP_${res.statusCode}`)));
        });
    ...
    async function getQuote(symbol) {
      return parseTencentQuote(await upstream(`https://qt.gtimg.cn/q=${symbol.upstream}`), symbol);
    }
    function json(url) {
      return new Promise((resolve, reject) => {
        const req = https.get(url, { headers: { "User-Agent": "Mozilla/5.0 stock-analysis-local-proxy" }, timeout: 8e3 }, (res) => {
          ...
          res.on("end", () => {
            try {
              res.statusCode === 200 ? resolve(JSON.parse(b)) : reject(new Error(`UPSTREAM_HTTP_${res.statusCode}`));
            } catch {
    ...
    async function getMinute(symbol) {
      const q = await getQuote(symbol), j = await json(`https://web.ifzq.gtimg.cn/appstock/app/minute/query?code=${symbol.upstream}`), raw = j?.data?.[symbol.upstream]?.data?.data || [];
      const points = raw.map((row) => {
        const a = String(row).split(" ");
        return { time: a[0], price: Number(a[1]), volume: Number(a[2]), amount: Number(a[3]) };
      }).filter((x) => Number.isFinite(x.price) && x.price > 0 && continuous(x.time));
      if (points.length < 1) throw new Error("UPSTREAM_EMPTY");
      let totalVolume = 0, totalAmount = 0;
      return { symbol: symbol.code, market: symbol.market, name: q.name, previousClose: q.previousClose, tradingDate: q.quoteTime.slice(0, 8), points: points.map((x) => {
        totalVolume += x.volume || 0;
        totalAmount += x.amount || 0;
        return { ...x, averagePrice: totalVolume ? totalAmount / totalVolume : null };
      }), source: "Tencent minute", quoteTime: q.quoteTime };
    }
    async function getDaily(symbol, count) {
      const q = await getQuote(symbol), j = await json(`https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=${symbol.upstream},day,,,${count},qfq`), box = j?.data?.[symbol.upstream], raw = box?.qfqday || box?.day || [];
      const rows = raw.map((a) => ({ date: a[0], open: Number(a[1]), close: Number(a[2]), high: Number(a[3]), low: Number(a[4]), volume: Number(a[5]) })).filter((x) => Number.isFinite(x.close) && x.close > 0);
      if (!rows.length) throw new Error("UPSTREAM_EMPTY");
      return { symbol: symbol.code, market: symbol.market, name: q.name, rows, source: "Tencent qfq kline", quoteTime: q.quoteTime };
    }
    async function getOrderbook(symbol) {
      const q = await getQuote(symbol);
      const bid = q.book?.filter((x) => x.side === "bid") || [], ask = q.book?.filter((x) => x.side === "ask") || [];
      return { symbol: symbol.code, market: symbol.market, name: q.name, bids: bid, asks: ask, spread: bid[0] && ask[0] ? ask[0].price - bid[0].price : null, source: "Tencent quote", quoteTime: q.quoteTime };
    }
    module.exports = { getQuote, getMinute, getDaily, getOrderbook };
  }
});

// shared/server/server/utils/decode-upstream.js
var require_decode_upstream = __commonJS({
  "shared/server/server/utils/decode-upstream.js"(exports, module) {
    "use strict";
    var iconv = require_lib();
    var SOURCE_ENCODINGS = { tencentQuote: ["gb18030", "gbk", "utf8"], tencentMinute: ["utf8", "gb18030"], domesticIndices: ["gb18030", "utf8"], globalIndices: ["utf8", "gb18030"], disclosures: ["utf8", "gb18030"], news: ["utf8", "gb18030"], eastmoney: ["utf8", "gb18030"] };
    var charset = (v) => String(v || "").match(/charset\s*=\s*["']?([^\s;"']+)/i)?.[1]?.toLowerCase();
    var jsonp = (v) => String(v).trim().replace(/^[\w$]+\s*\(/, "").replace(/\)\s*;?$/, "");
    function decodeUpstreamResponse({ buffer, contentType, source, preferredEncodings }) {
      const preferred = preferredEncodings || SOURCE_ENCODINGS[source] || ["utf8", "gb18030", "gbk"];
      const declared = charset(contentType);
      const list = [declared, ...preferred].filter((v, i, a) => v && a.indexOf(v) === i);
      let best = "";
      for (const enc of list) {
        if (!iconv.encodingExists(enc)) continue;
        const text = iconv.decode(buffer, enc);
        if (!best || text.split("\uFFFD").length < best.split("\uFFFD").length) best = text;
        if (!text.includes("\uFFFD")) {
          best = text;
          break;
        }
      }
      return { text: best.replace(/^\uFEFF/, ""), encoding: list.find((enc) => iconv.encodingExists(enc) && iconv.decode(buffer, enc) === best) || "utf8", isJsonp: /^[\w$]+\s*\(/.test(best.trim()), isHtml: /<\s*html|<!doctype/i.test(best) };
    }
    module.exports = { decodeUpstreamResponse, SOURCE_ENCODINGS, jsonp };
  }
});

// shared/server/server/market/external.js
var require_external = __commonJS({
  "shared/server/server/market/external.js"(exports, module) {
    "use strict";
    var https = __require("https");
    var { normalizeSymbol } = require_symbol();
    var { decodeUpstreamResponse, jsonp } = require_decode_upstream();
    ...
    var request = (url, source) => new Promise((resolve, reject) => {
      const started = Date.now();
      const r = https.get(url, { headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120 Safari/537.36", "Referer": "https://quote.eastmoney.com/", "Accept": "application/json,text/plain,*/*" } }, (res) => {
        const chunks = [];
        res.on("data", (d) => chunks.push(d));
        res.on("end", () => {
          const buffer = Buffer.concat(chunks), meta = { status: res.statusCode, contentType: res.headers["content-type"] || "", contentEncoding: res.headers["content-encoding"] || "", bytes: buffer.length, durationMs: Date.now() - started };
          if (res.statusCode < 200 || res.statusCode >= 300) return reject(new UpstreamError("UPSTREAM_HTTP_" + res.statusCode, "Upstream HTTP " + res.statusCode));
          const decoded = decodeUpstreamResponse({ buffer, contentType: meta.contentType, source });
          resolve({ ...decoded, meta });
        });
      });
      r.setTimeout(8e3, () => r.destroy(new UpstreamError("UPSTREAM_TIMEOUT", "Upstream timeout")));
      r.on("error", (e) => reject(e instanceof UpstreamError ? e : new UpstreamError("UPSTREAM_NETWORK", e.message)));
    });
    var parse = async (url, source) => {
      const r = await request(url, source);
      if (r.isHtml) throw new UpstreamError("UPSTREAM_HTML", "Upstream returned HTML");
      try {
    ...
    var secid = (s) => `${s.market === "SH" ? "1" : "0"}.${s.code}`;
    async function capitalFlow(s) {
      const j = await parse(`https://push2his.eastmoney.com/api/qt/stock/fflow/daykline/get?lmt=1&klt=101&secid=${secid(s)}&fields1=f1,f2,f3,f7&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61,f62,f63`, "eastmoney");
      const v = j?.data?.klines?.[0]?.split(",");
      if (!v) throw new UpstreamError("EMPTY_FLOW", "No capital-flow record", false);
      return { symbol: s.code, main: Number(v[1]), superLarge: Number(v[2]), large: Number(v[3]), medium: Number(v[4]), small: Number(v[5]), source: "Eastmoney fund flow" };
    }
    async function disclosures(s, limit) {
      const j = await parse(`https://np-anotice-stock.eastmoney.com/api/security/ann?sr=-1&page_size=${limit}&page_index=1&ann_type=A&client_source=web&stock_list=${s.code}`, "disclosures");
      return { symbol: s.code, items: (j?.data?.list || []).map((x) => ({ title: x.title || "", date: String(x.notice_date || x.display_time || "").slice(0, 10), columns: (x.columns || []).map((c) => c.column_name).join("/"), url: `https://data.eastmoney.com/notices/detail/${s.code}/${x.art_code || ""}.html` })).filter((x) => x.title), source: "Eastmoney announcements" };
    }
    async function tencent(codes, source, encodingSource = "tencentQuote") {
      const r = await request("https://qt.gtimg.cn/q=" + codes.join(","), encodingSource);
      const rows = r.text.split(";").map((x) => {
        const m = x.match(/v_([^=]+)="([^"]*)/);
        if (!m) return null;
        const a = m[2].split("~");
        return { symbol: m[1], name: a[1] || "", price: Number(a[3]), previousClose: Number(a[4]), changePercent: Number(a[32]), quoteTime: a[30] || "" };
      }).filter(Boolean);
    ...
    async function fundamentals(s) {
      ...
      const x = r.items[0];
      if (!x || !Number.isFinite(x.price)) throw new UpstreamError("EMPTY_FUNDAMENTALS", "No fundamentals quote", false);
      return { ...x, symbol: s.code, pe: null, pb: null, marketCap: null, source: r.source };
    }
    var domestic = () => tencent(["sh000001", "sz399001", "sz399006"], "Tencent domestic indices", "domesticIndices");
    var global = () => tencent(["usIXIC", "hkHSI"], "Tencent global indices", "globalIndices");
    async function news(s, limit) {
      const j = await parse(`https://search-api-web.eastmoney.com/search/jsonp?param=%7B%22uid%22%3A%22%22%2C%22keyword%22%3A%22${encodeURIComponent(s.code)}%22%2C%22type%22%3A%5B%22cmsArticleWebOld%22%5D%2C%22pageindex%22%3A1%2C%22pagesize%22%3A${limit}%7D`, "news");
      const list = j?.result?.cmsArticleWebOld || [];
      if (!list.length) throw new UpstreamError("EMPTY_NEWS", "No news record", false);
      return { symbol: s.code, items: list.map((x) => ({ title: x.title, date: x.date || "", source: x.mediaName || "Eastmoney", url: x.url || "" })), source: "Eastmoney news search" };
    }
    module.exports = { capitalFlow, disclosures, fundamentals, domestic, global, news, UpstreamError, request };
  }
});

// shared/server/server/core/router.js
var require_router = __commonJS({
  "shared/server/server/core/router.js"(exports, module) {
    "use strict";
    var { normalizeSymbol } = require_symbol();
    var { getQuote, getMinute, getDaily, getOrderbook } = require_quote();
    var external = require_external();
    var failure = (code, message, retryable = false, source = "local-proxy") => ({ success: false, data: null, source, fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), cacheHit: false, stale: false, error: { code, message, retryable } });
    var success = (data) => ({ success: true, data, source: data.source || "", fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), cacheHit: false, stale: false });
    function log(meta) {
      console.info(JSON.stringify({ requestId: meta.requestId || "", route: meta.pathname, symbol: meta.symbol || "", durationMs: Date.now() - meta.started, result: meta.result || "", errorCode: meta.errorCode || "", cacheHit: false, source: meta.source || "" }));
    }
    async function route2({ method = "GET", url, requestId = "", clientIp = "", region = "" }) {
      const parsed = new URL(url, "http://localhost"), pathname = parsed.pathname, started = Date.now();
      if (method === "OPTIONS") return { status: 204, body: null, headers: {} };
      if (method !== "GET") return { status: 405, body: failure("METHOD_NOT_ALLOWED", "GET or OPTIONS required"), headers: { Allow: "GET, OPTIONS" } };
      if (pathname === "/health") return { status: 200, body: { success: true, service: "stock-analysis-local-proxy", environment: process.env.NODE_ENV || "development" }, headers: {} };
      if (pathname === "/ready") return { status: 200, body: { success: true, ready: true }, headers: {} };
      if (pathname === "/api/platform-check") return { status: 200, body: { success: true, data: { node: process.version, buffer: typeof Buffer !== "undefined", iconvLite: true, requestId: requestId || null, region: region || null, clientIpPresent: Boolean(clientIp) }, source: "platform-check", fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), cacheHit: false, stale: false }, headers: {} };
      const limit = Math.min(20, Math.max(1, Number(parsed.searchParams.get("limit")) || 20));
      const global = { "/api/indices/domestic": external.domestic, "/api/indices/global": external.global };
      try {
        if (global[pathname]) {
          const data2 = await global[pathname]();
          log({ pathname, requestId, started, result: "success", source: data2.source });
          return { status: 200, body: success(data2), headers: {} };
        }
        const handlers = { "/api/quote": getQuote, "/api/minute": getMinute, "/api/orderbook": getOrderbook, "/api/daily": (s) => getDaily(s, Math.min(240, Math.max(60, Number(parsed.searchParams.get("count")) || 180))), "/api/capital-flow": external.capitalFlow, "/api/disclosures": (s) => external.disclosures(s, limit), "/api/news": (s) => external.news(s, limit), "/api/fundamentals": external.fundamentals };
        const handler = handlers[pathname];
        if (!handler) return { status: 404, body: failure("NOT_FOUND", "Route not found"), headers: {} };
        const symbol = normalizeSymbol(parsed.searchParams.get("symbol"));
        if (!symbol) return { status: 400, body: failure("INVALID_SYMBOL", "A six-digit Shanghai, Shenzhen, or Beijing symbol is required"), headers: {} };
        const data = await handler(symbol);
        log({ pathname, requestId, symbol: symbol.code, started, result: "success", source: data.source });
        return { status: 200, body: success(data), headers: {} };
      } catch (e) {
        log({ pathname, requestId, started, result: "unavailable", errorCode: e.code || "UPSTREAM_UNAVAILABLE", source: "external-source" });
        return { status: 200, body: failure(e.code || "UPSTREAM_UNAVAILABLE", e.message, e.retryable !== false, "external-source"), headers: {} };
      }
    }
    module.exports = { route: route2, failure };
  }
});

// handler-source.js
var import_router = __toESM(require_router(), 1);
var { route } = import_router.default;
